...
changeToRandomQuoate();
function changeToRandomQuoate(){
  textElement.innerText = Array(10).fill(0).map(e=>wordList[Math.floor(Math.random()* wordList.length)]).join(' ');}
...
inputBox.addEventListener("input", () => {
  ...
  if (typedText.length === 1) {
    ...
    interval = setInterval(updateSpeedAndAccuracy, 1000);
  }
...
function updateSpeedAndAccuracy() {
  ...
  const correctChars = [...typedText].filter((char, i) => char === originalText[i]).length;
  ...
  if (typedText === originalText) clearInterval(interval);
}
